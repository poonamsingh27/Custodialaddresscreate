...
contract TronCustodialfactory {
    ...
    mapping(address => mapping(uint256 => address)) private wallets;
  ...
  function create(address owner, uint256 index) public returns (address) {
   require(wallets[owner][index] == address(0), "Wallet already exists");
  // TronCustodialWallet newTronCustodialWallet = new TronCustodialWallet();
    address addr = address(new TronCustodialWallet());
    wallets[owner][index] = address(addr);
   ...
   function createBatch(address owner, uint256[] memory index) external {
    ...
    for (uint256 i = 0; i < index.length; i++) {
        ...
        require(wallets[owner][index[i]] == address(0), "Wallet already exists");
        
        address addr = address(new TronCustodialWallet());
         wallets[owner][index[i]] = addr;
        TronCustodialWallet(payable(addr)).init(owner);
        emit Created(addr);
 ...
 function getWallet(address owner, uint256 index) public view returns (address) {
        return wallets[owner][index];
    }
    
  function getWallets(address owner, uint256[] memory index) external view returns (address[] memory) {
    require(index.length <= _MAX_ARRAY_BOUNDS, "Maximum allowable size of array has been exceeded");

    address[] memory walletAddresses = new address[](index.length);
    for (uint256 i = 0; i < index.length; i++) {
        walletAddresses[i] = wallets[owner][index[i]];
    }
